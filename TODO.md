...
- [ ] Implement signal_invalidation_exit — store entry snapshot conditions per position; re-evaluate daily; exit when fully reversed
...
- [ ] Improve iv_compression — loosen hv_rank_max from 0.10 to 0.15; add short-side variant (below SMA200 + IV compression)
- [ ] Improve insider_buying — distinguish open-market vs option exercise; scale by purchase size vs compensation; require ≥3 insiders in 5 days for highest tier
...
- [ ] Build data/pairs.py — sector cointegration testing (Engle-Granger), z-score tracking, pair selection and refresh logic
- [ ] Set up FinBERT NLP pipeline — install transformers + torch CPU, download ProsusAI/finbert, build inference wrapper for EDGAR 8-K classification
