...
| `residual_reversal` | Market-excess 5d return (stock − SPY) ≤ −7% **and** sector-excess 5d return (stock − own-sector EW peers) ≤ −7% (v1.144 conjunct — filters sector-rout beta; fail-open when sector unknown) — idiosyncratic laggard reverting over 1–3d. Blocked in HIGH_VOL_DOWNTREND (train-negative) and UNKNOWN; **v1.145: UNBLOCKED in STRESS_RISK_OFF for liquid names** (`spread_proxy_20d ≤ spread_proxy_max`) — that capitulation-bounce bucket is reversal's strongest (+1.86%/3d @7bps, robust both walk-forward halves); the liquidity gate is the survivorship guardrail (2026-07 workshop, N1) | 3 days |
